...
type MessageQuery struct {
	...
	withFrom      *MessageQuery
	withTo        *MessageQuery
	modifiers     []func(*sql.Selector)
	loadTotal     []func(context.Context, []*Message) error
	withNamedFrom map[string]*MessageQuery
	withNamedTo   map[string]*MessageQuery
...
func (mq *MessageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Message, error) {
	...
	if query := mq.withFrom; query != nil {
		if err := mq.loadFrom(ctx, query, nodes,
			func(n *Message) { n.Edges.From = []*Message{} },
			func(n *Message, e *Message) { n.Edges.From = append(n.Edges.From, e) }); err != nil {
			return nil, err
		}
	}
	if query := mq.withTo; query != nil {
		if err := mq.loadTo(ctx, query, nodes,
			func(n *Message) { n.Edges.To = []*Message{} },
			func(n *Message, e *Message) { n.Edges.To = append(n.Edges.To, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range mq.withNamedFrom {
		if err := mq.loadFrom(ctx, query, nodes,
			func(n *Message) { n.appendNamedFrom(name) },
			func(n *Message, e *Message) { n.appendNamedFrom(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range mq.withNamedTo {
		if err := mq.loadTo(ctx, query, nodes,
			func(n *Message) { n.appendNamedTo(name) },
			func(n *Message, e *Message) { n.appendNamedTo(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (mq *MessageQuery) loadFrom(ctx context.Context, query *MessageQuery, nodes []*Message, init func(*Message), assign func(*Message, *Message)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Where(sql.InValues(joinT.C(message.FromPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(message.FromPrimaryKey[1]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "from" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (mq *MessageQuery) loadTo(ctx context.Context, query *MessageQuery, nodes []*Message, init func(*Message), assign func(*Message, *Message)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uuid.UUID]*Message)
	nids := make(map[uuid.UUID]map[*Message]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Where(sql.InValues(joinT.C(message.ToPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(message.ToPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "to" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
// WithNamedFrom tells the query-builder to eager-load the nodes that are connected to the "from"
...
func (mq *MessageQuery) WithNamedFrom(name string, opts ...func(*MessageQuery)) *MessageQuery {
	...
	if mq.withNamedFrom == nil {
		mq.withNamedFrom = make(map[string]*MessageQuery)
	}
	mq.withNamedFrom[name] = query
	return mq
}

// WithNamedTo tells the query-builder to eager-load the nodes that are connected to the "to"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (mq *MessageQuery) WithNamedTo(name string, opts ...func(*MessageQuery)) *MessageQuery {
	...
	if mq.withNamedTo == nil {
		mq.withNamedTo = make(map[string]*MessageQuery)
	}
	mq.withNamedTo[name] = query
	return mq
}
